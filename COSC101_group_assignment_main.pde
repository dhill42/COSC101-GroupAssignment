...
void gunManager() {
  ...
  for (int i = 0; i < magazineSize; i++) { // Loop through all active shots
    if (shots[i][0] != null) {
      ...
      // Check if shot has reached mouse position that was stored on mouse click
      float distance = PVector.dist(shots[i][0], shots[i][1]);
      if (distance < 5) {
        drawExplosion(shots[i][1], 30); // Draw the explosion
        shots[i][0] = null; // Remove the shot projectile        
      }
    }
  }
}
/* This drawExplosion function draws an explosion when the shot reaches the crosshair location. */
void drawExplosion(PVector explosionPos, float radius) {
    fill(255,0,0); // Set the fill color of the circle to red, with decreasing opacity
    ellipse(explosionPos.x, explosionPos.y, 80, 80); // Draw the circle at the explosion position with the current radius
}

/* MISSILE FUNCTION HERE */
// ...
/* COLLISION FUNCTION HERE (IF REQUIRED SEPARATELY) */
// ...
// Mirror the drawExplosion circle for drawing a detection circle
// Trigger detection when missile enters this circle.
// ...
/*************************************************************************************************
* Mouse Controls
*************************************************************************************************/
